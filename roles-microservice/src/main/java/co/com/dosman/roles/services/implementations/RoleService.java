...
public class RoleService implements IRoleService {
    ...
    @Override
    public String deleteRole(Long id) throws RoleException, NotFoundException {
        if (id == null) throw new RoleException(RoleValidate.ID_NOT_NULL);
        if (!roleRepository.existsById(id)) throw new NotFoundException(RoleValidate.ROLE_NOT_FOUND);
    ...
    @Override
    public GetRoleDTO getRoleById(Long id) throws RoleException, NotFoundException {
        if (id == null) throw new RoleException(RoleValidate.ID_NOT_NULL);
        if (!roleRepository.existsById(id)) throw new NotFoundException(RoleValidate.ROLE_NOT_FOUND);
    ...
    @Override
    public GetRoleDTO saveRole(CreateRoleDTO createRoleDTO) throws RoleException, ConflictException {
        if (createRoleDTO == null) throw new RoleException(RoleValidate.ROLE_NOT_NULL);
        if (roleRepository.existsByName(createRoleDTO.getName())) throw new ConflictException(RoleValidate.NAME_ALREADY_EXISTS);

        return RoleMapper.modelToGetRoleDto(roleRepository.save(RoleMapper.createRoleDtoToModel(createRoleDTO)));
    }

    @Override
    public GetRoleDTO updateRole(UpdateRoleDTO updateRoleDTO) throws RoleException, NotFoundException {
        if (updateRoleDTO == null) throw new RoleException(RoleValidate.ROLE_NOT_NULL);
        if (!roleRepository.existsById(updateRoleDTO.getId())) throw new NotFoundException(RoleValidate.ROLE_NOT_FOUND);
